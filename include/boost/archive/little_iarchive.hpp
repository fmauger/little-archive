 ...
 *
 * Based on the portable binary example by Robert Ramey this implementation
 ...
 *       user defined type to be used with the little archives are required 
...
#include "little_archive_exception.hpp"

namespace boost { namespace archive {

	// forward declaration
	class little_iarchive;
	 ...
	 * in a little_archive_exception if loaded into an int on a 32 bit system.
	 ...
	 */
	class little_iarchive : public portable_iprimitive
		...
		, public basic_binary_iarchive<little_iarchive>
	{
		// only needed for Robert's hack in basic_binary_iarchive::init
		friend class basic_binary_iarchive<little_iarchive>;
	...
	public:
		 ...
		 */
		little_iarchive(std::istream& is, unsigned flags = 0)
			: portable_iprimitive(*is.rdbuf(), flags & no_codecvt)
			, basic_binary_iarchive<little_iarchive>(flags)
		{
			init(flags);
		}

		little_iarchive(std::streambuf& sb, unsigned flags = 0)
		...
		void load(bool& b)
		{
			...
			default: throw little_archive_exception(c);
			}
		}
			...
			load(T & t, dummy<2> = 0)
		{
			...
			else t = 0; // zero value optimization
		}
			...
			load(T & t, dummy<3> = 0)
		{
			...
			if (get_flags() & no_infnan && !fp::isfinite(t))
				throw little_archive_exception(t);
	...
	typedef detail::polymorphic_iarchive_route<little_iarchive> polymorphic_little_iarchive;

} } // namespace boost::archive

// this is required by export which registers all of your
// classes with all the inbuilt archives plus our archive.
BOOST_SERIALIZATION_REGISTER_ARCHIVE(boost::archive::little_iarchive)
BOOST_SERIALIZATION_REGISTER_ARCHIVE(boost::archive::polymorphic_little_iarchive)
...
namespace boost { namespace archive {
	...
	template class basic_binary_iarchive<little_iarchive>;
	...
	template class detail::archive_serializer_map<little_iarchive>;
